...
export default function NewBannerForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Symulacja dodawania banera
      await new Promise(resolve => setTimeout(resolve, 1000))

      setMessage('Banner został dodany pomyślnie!')
      setTimeout(() => {
        router.push('/admin/banners')
      }, 1500)
    } catch (error) {
      setMessage('Błąd podczas dodawania banera!')
    } finally {
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Dodaj nowy banner</h1>
            <p className="mt-2 text-gray-600">Wypełnij formularz aby dodać nowy banner</p>
          </div>
        </div>
      </div>
